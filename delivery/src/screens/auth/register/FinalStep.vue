<template>
    <nb-container>
        <Spinner
          :visible="isLoading"
        />
        <header :navigation="navigation" headerTitle="Registration" :registration=true />
        <nb-content padder>
            <touchable-opacity class="photo-container" :on-press="()=> openImageDialog('featuredImage')">
                ...
                <nb-view class="image-upload" v-else>
                    <image :source="require('../../../../assets/images/user/upload.png')" class="upload-icon" />
                    <nb-text class="image-upload-head">Upload Profile Image</nb-text>
                    <nb-text class="image-upload-subhead">Please upload your Profile image </nb-text>
                </nb-view>
            </touchable-opacity> 
            <touchable-opacity class="photo-container" :on-press="()=> openImageDialog('proofFrontImage')">
                ...
                <nb-view class="image-upload" v-else>
                    ...
                    <nb-text class="image-upload-subhead">Please upload front page of your Driving Licence</nb-text>
                </nb-view>
            </touchable-opacity>
            <touchable-opacity class="photo-container" :on-press="()=> openImageDialog('proofBackImage')">
                ...
                <nb-view class="image-upload" v-else>
                    <image :source="require('../../../../assets/images/user/upload.png')" class="upload-icon" />
                    <nb-text class="image-upload-head">Upload Document</nb-text>
                    <nb-text class="image-upload-subhead">Please upload back page of your Driving Licence</nb-text>
                </nb-view>
            </touchable-opacity>
               <nb-item regular class="input-item">
                <nb-input placeholder="Driving Licence Number" :on-change-text="(text)=>{drivingLicenceNumber=text}" :defaultValue="drivingLicenceNumber"/>
            </nb-item>
            <nb-button block rounded :on-press="uploadFiles"><nb-text>Next</nb-text></nb-button>
        </nb-content>
        <Dialog
                ...
                <nb-view>
                    <nb-view class="upload-dialog">
                        <touchable-opacity class="upload-camera" :on-press="()=>captureImage(imageType)">
                            <image class="camera-icon" :source="require('../../../../assets/images/user/camera.png')"/>
                            <nb-text class="camera-text">Camera</nb-text>
                        </touchable-opacity>
                        <touchable-opacity class="upload-image" :on-press="()=>pickImage(imageType)">
                            <image class="camera-icon" :source="require('../../../../assets/images/user/upload-photo.png')"/>
...
export default {
    ...
    methods: {
        ...
        uploadFiles(){
            ...
            formData.append('drivingLicenceNumber', this.drivingLicenceNumber)
            ...
            .then(response => { 
                this.$store.commit('user/setImage', {
