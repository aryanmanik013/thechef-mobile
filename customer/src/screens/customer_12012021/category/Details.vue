        ...
        <nb-content> 
            <nb-tabs :tabBarUnderlineStyle="{height:3, marginTop:1}">
                <nb-tab heading="Menu" :on-press="()=> clickTab('menu')" :textStyle="{fontSize: 14, color: '#8c8c8c'}" :activeTextStyle="{fontSize: 14, color: '#000000'}">
                    <nb-content padder>
                        <nb-list v-if="getAllMenus&&allMenus">
                            <nb-list-item  v-for="menus in allMenus" :key="menus.id" avatar noBorder class="menu-list-item" :on-press="()=>{ getMenuDetail(menus.id)}">
                                <nb-left :style="{ marginTop:0, paddingTop:0 }">
                                    <image-background :imageStyle="{borderRadius: 60}" :source=" menus.image ? {uri:menus.image} :require('../../../../assets/images/menu/chicken-biriyani.png')" class="product-thumb">
                                        <image v-if="menus.veg_status == '0'" :source="require('../../../../assets/images/icons/veg.png')" class="food-type"/>
                                ...
                                <nb-body :style="{ marginTop:0, paddingTop:0, marginLeft:8}">
                                    <nb-text class="menu-item-name">{{menus.name}}</nb-text>
                                    <nb-text class="menu-item-kitchen">{{menus.kitchen_name}}</nb-text>
                                    <nb-text class="menu-item-description" :numberOfLines="2">{{menus.description}}</nb-text>
                                    <nb-text class="menu-item-order-time">Order Time <nb-text class="menu-item-time">{{menus.order_time}}</nb-text></nb-text>
                                    <nb-view class="price-section">
                                        <nb-text class="menu-item-price">₹{{parseFloat(menus.price).toFixed(2)}}</nb-text>
                                        <nb-text class="menu-item-quantity">{{menus.quantity}} <nb-text class="menu-item-quantity-remaining">remaining</nb-text></nb-text>
                                        <nb-view class="menu-item-distance">
                                            <image :source="require('../../../../assets/images/icons/distance.png')" class="menu-item-distance-icon" />
                                            <nb-text class="menu-item-distance-text">{{menus.distance.toFixed(2)}}</nb-text>
                                        </nb-view>
                                    </nb-view>
                                </nb-body>
                            </nb-list-item>
                ...
                <nb-tab heading="Kitchen" :on-press="()=> clickTab('kitchen')" :textStyle="{fontSize: 14, color: '#8c8c8c'}" :activeTextStyle="{fontSize: 14, color: '#000000'}">
                    <nb-content padder v-if="getAllMenus&&allKitchens">
                 
                        <nb-view  v-for="kitchens in allKitchens" :key="kitchens.id" class="kitchen-list-item">
                         <touchable-opacity  :on-press="()=> {navigation.navigate('KithenDetail',{kitchenId:kitchens.id})}">
                            <image :source="kitchens.image ? {uri:kitchens.image} :require('../../../../assets/images/home/kitchen.png')" class="kitchen-image" :style="{width:kitchenImageWidth, height:kitchenImageWidth*.4283}"/>
                            <nb-view padder class="kitchen-detail">
                                <nb-view class="kitchen-head">
                                    <nb-text class="kitchen-name">{{kitchens.name}}</nb-text>
                                    <nb-view class="rating">
                                        <image :source="require('../../../../assets/images/icons/star-rating.png')" class="rating-icon" />
                                        <nb-text class="rating-text">{{kitchens.rating}}</nb-text>
                                    </nb-view>
                                </nb-view>
                                <nb-view class="kitchen-desc">
                                    <nb-text class="kitchen-description" :numberOfLines="1">{{kitchens.description}}</nb-text>
                                    <nb-view class="kitchen-distance">
                                        <image :source="require('../../../../assets/images/icons/distance.png')" class="kitchen-distance-icon" />
                                        <nb-text class="kitchen-distance-text">{{kitchens.distance.toFixed(2)}}</nb-text>
                                    </nb-view>
                                </nb-view>
                            </nb-view>
                          </touchable-opacity>
