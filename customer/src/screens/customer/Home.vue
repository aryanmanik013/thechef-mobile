<template>
    <nb-container padder>
        ...
        <nb-view class="search-container">
            <nb-item class="search-box" :on-press="()=>{navigation.navigate('Search',{search:true,searchType:'searching'})}">
                <image :source="require('../../../assets/images/icons/search.png')" class="search-icon" />
                <nb-input class="search-input" pointerEvents="none" :editable="false" :on-change-text="(text)=>{search=text}" :defaultValue="search" placeholder="Search for Item or kitchen" />
            </nb-item>
        </nb-view>
        <nb-content>
            <scroll-view v-if="allBanners" :horizontal=true :showsHorizontalScrollIndicator=false :style="{marginLeft:15}" >
                 <touchable-opacity v-for="banner in allBanners" :key="banner.id" :on-press="()=>{ navigation.navigate(banner.activity,banner.parameter)}" class="home-banner-item">
                    <image :source="banner.image ? {uri:banner.image} : require('../../../assets/images/home/banner1.png')" class="home-banner-image" :style="{width:bannerImageWidth,height:bannerImageWidth*0.515}"/>
                </touchable-opacity>
            </scroll-view>
            <nb-view v-if="allCategories" padder class="category">
                <nb-view class="category-header">
                    <nb-text class="category-title">Popular Category</nb-text>
                    <touchable-opacity class="category-button" :on-press="()=>{navigation.navigate('CategoryList')}">
                        <nb-text class="category-button-text">See all</nb-text>
                    </touchable-opacity>
                </nb-view>
                <nb-view  class="home-category">
                       <touchable-opacity v-for="category in allCategories" :key="category.id" :on-press="()=>{navigation.navigate('CategoryDetail',{categoryId:category.id,searchType:'category'})}">
                        <nb-view class="category-item" >
            ...
            <nb-tabs :tabBarUnderlineStyle="{height:3, marginTop:1}" :onChangeTab="(tab) => {
                allKitchens=[]
                allMenus=[]
                tab.ref.props.index=='menu' ? getAllMenus() : getAllKitchens()
                }">
                <nb-tab heading="Menu" index="menu" :textStyle="{fontSize: 14, color: '#8c8c8c'}" :activeTextStyle="{fontSize: 14, color: '#000000'}">
                    <!-- <nb-view padder>
                ...
                <nb-tab heading="Kitchen" index="kitchen" :textStyle="{fontSize: 14, color: '#8c8c8c'}" :activeTextStyle="{fontSize: 14, color: '#000000'}">
                    <nb-view padder v-if="allKitchens" >
                  ...
                  <nb-tab heading="Pre order" index="menu" :textStyle="{fontSize: 14, color: '#8c8c8c'}" :activeTextStyle="{fontSize: 14, color: '#000000'}">
                    <nb-view padder>
                        <nb-list v-if="allMenus">
                            <nb-list-item  v-for="menu in allMenus" :key="menu.id" avatar noBorder class="menu-list-item" :on-press="()=>{ getMenuDetail(menu.id)}"  :style="{ shadowColor: '#000',shadowOffset: { width: 0, height: 1 },shadowOpacity: 0.2,shadowRadius: 1.2}">
                                <nb-left :style="{ marginTop:0, padding:6, paddingRight:0, paddingTop:6, justifyContent:'center', alignItems:'center' }">
                                ...
                                <nb-body :style="{ marginTop:0, paddingTop:3, marginLeft:8}">
                                    ...
                                    <nb-text class="menu-item-description" :numberOfLines="2" >{{menu.description}}</nb-text>
                                    <nb-text class="menu-item-order-time">Order Time <nb-text class="menu-item-time">{{menu.order_time}}</nb-text></nb-text>
                                     <nb-text class="menu-item-order-time">Order Date <nb-text class="menu-item-time">{{menu.order_date}}</nb-text></nb-text>
                                    <nb-view class="price-section">
                                        <nb-text class="menu-item-price">₹{{parseFloat(menu.price).toFixed(2)}}</nb-text>
                                        <nb-text class="menu-item-quantity">{{menu.quantity}} <nb-text class="menu-item-quantity-remaining">remaining</nb-text></nb-text>
                                        <nb-view class="menu-item-distance">
...
export default {
    ...
    beforeMount(){ 
        // alert(this.$store.state.token.token)
        var routeName = ''
        if(!this.$store.state.user.loggedIn && !this.$store.state.user.verificationStatus && this.$store.state.user.authentication){
            routeName = 'ConfirmOTP'
        } else if(!this.$store.state.skipLogin && !this.$store.state.user.loggedIn && !this.$store.state.splash){
            routeName = 'Verify' 
        }
        if(routeName){
            if(routeName=='OTPConfirm'){
                const resetAction = StackActions.reset({
                    index: 0,
                    actions: [NavigationActions.navigate({ routeName: routeName, params:{mobile:this.$store.state.user.mobile, otp:''} })]
                })
    ...
    methods: {
        getAllBanners(){
            ...
            this.api.create({
                method: 'POST',
                url: this.api.routes.allBanners,
        ...
        getAllCategories(){
            ...
            this.api.create({
                method: 'POST',
                url: this.api.routes.popularCategories,
                headers: { 'Content-Type': 'application/json' },
                data:{
                    latitude: this.$store.state.selectedLocation.coords.latitude,
                    longitude: this.$store.state.selectedLocation.coords.longitude
        ...
        getAllKitchens(){
            ...
            this.api.create({
                method: 'POST',
                url: this.api.routes.allKitchens,
                headers: { 'Content-Type': 'application/json' },
                data:{
                    latitude: this.$store.state.selectedLocation.coords.latitude,
                    longitude: this.$store.state.selectedLocation.coords.longitude
