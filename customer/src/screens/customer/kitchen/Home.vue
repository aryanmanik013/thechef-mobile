    ...
    <nb-container padder>
        ...
        <nb-view class="search-container">
            <nb-item class="search-box">
                ...
                <nb-input class="search-input" :on-change-text="(text)=>{search=text}" :defaultValue="search" :on-focus="()=>{navigation.navigate('Search',{search:true,searchType:'searching'})}" placeholder="Search for Item or kitchen" />
            </nb-item>
        </nb-view>
        <nb-content>
            <scroll-view v-if="allBanners"  :horizontal=true :showsHorizontalScrollIndicator=false :style="{marginLeft:15}" >
                 <touchable-opacity v-for="banner in allBanners" :key="banner.id" :on-press="()=>{ navigation.navigate(banner.activity,{parameter})}" class="home-banner-item">
            ...
            <nb-view v-if="allCategories" padder class="category">
                ...
                <nb-view  class="home-category">
                       <touchable-opacity v-for="category in allCategories" :key="category.id" :on-press="()=>{navigation.navigate('CategoryDetail',{categoryId:category.id})}">
