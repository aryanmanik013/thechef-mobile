<template>
    <nb-container>
        <Spinner
            :visible="isLoading"
        />
        <nb-view :style="{position:'absolute', zIndex:2, width:'100%', marginBottom:15}">
            ...
            <nb-view class="address-form" padder>
                ...
                <InputWithError
                    ...
                    <InputWithError
                        ...
                        :msg="errors.mobile!='' ? errors.mobile : ($v.form.mobile.required && !$v.form.mobile.$numeric) ? 'Mobile number entered is invalid' : 'Mobile number field is required'">
                        <nb-label class="input-label">Mobile Number</nb-label>
                        <nb-input keyboardType="phone-pad" class="input-field" :on-change-text="(text)=>{form.mobile=text}" :defaultValue="form.mobile" placeholder="Phone Number"
...
<script>
    ...
    export default {
        ...
        components: {
             MapView, Marker, GooglePlacesAutocomplete, MapViewDirections
        },
        data() {
            return {
    ...
    beforeMount(){
        ...
        this.countryCode=this.$store.state.selectedLocation.addressComponents.countryCode
        this.stateCode=this.$store.state.selectedLocation.addressComponents.stateCode
    },
    created() {
    ...
    methods: {
        ...
        changePlace(details) {
            ...
            .then(json => {
                var format_address = json.results[0].formatted_address;
                this.format_address=format_address;
                this.form.street=format_address;
